...
### AWS 의 컨테이너 오케스트레이션 서비스

- AWS 에서 컨테이너 시스템 구축하는 경우 2가지 선택지가 존재함
    - **Amazon Elastic Container Service (ECS)**
        - 컨테이너 오케스트레이션 기능을 **AWS 가 담당**
    - **Amazon Elastic Kubernetes Service (EKS)**
        - 컨테이너 오케스트레이션 기능을 **쿠버네티스가 담당**
- ECS
    - AWS 관리형 컨테이너 오케스트레이션 서비스
        - 컨테이너 배포, 상태 모니터링, 스케일링 관리, 작업 정의를 기반으로 설정된 **컨테이너 상태와 수를 유지**
        - `태스크` 정의 : 컨테이너 이미지, 실행 수와 같은 태스크 설정을 정의
            - ECS 에서 **컨테이너는 태스크**라는 개념으로 정의됨
        - `노드` : **컨테이너를 실행하는 서버**, ECS 의 경우 **EC2 인스턴스나 Fargate 선택 가능**
        - `서비스` : `ALB` (Application Load Balancer) 와 연결된 태스크의 집합체
    - **Amazon Elastic Container Registry (ECR)** 이라는 `컨테이너 이미지 레지스트리` 서비스도 제공하여, 사용자 정의된 컨테이너 이미지를 AWS 에서 관리하고 **ECS 에도 쉽게 배포 가능**
    
### EC2 와 Fargate

- ECS 를 이용해 컨테이너 기동 시 선택가능한 플랫폼은 EC2 와 Fargate 2가지이다
- `EC2` : EC2 인스턴스 내에서 실행되는 컨테이너 런타임에서 컨테이너 실행
    - 사용자가 **컨테이너 런타임이 설치된 EC2 를 관리해야 함**
    - 사용자 책임 범위 ; OS, 미들웨어, 컨테이너에서 실행되는 응용프로그램
    - CPU/메모리 늘리고 싶다면, 인스턴스 타입 변경 및 EBS 볼륨 크기 확장
- `Fargate` : AWS 에서 관리하는 서버에서 컨테이너 실행
    - AWS 에서 컨테이너 런타임, OS 버전업 같은 서버관리를 하니까 **사용자는 컨테이너 이미지만 관리하면 됨**
    - 컨테이너 런타임, OS 버전은 플랫폼 버전 (PV) 로 관리되므로, Fargate 실행 시 PV 를 지정해야 함
    - 사용자 책임 범위 ; 컨테이너에서 실행되는 응용프로그램
    - CPU/메모리 직접 할당가능하지만, 지정한 CPU에 따라 할당 가능한 메모리 크기가 정해져있으며, 스토리지는 Fargate 태스크 스토리지로 태스크 실행 시 할당되며 할당량은 PV에 따라 다름
<br>

- 기본적으로 AWS 에서 컨테이너를 사용해야 한다면 ECS 이용하는 게 가장 무난하지만, 구축하려는 시스템이 K8S 이용을 전제로 한다면 EKS 를 사용 해야 함

### 서버지식 없이 사용할 수 있는 대표적인 서비스 5개

- **AWS Lightsail**
    - 일반적으로 자주 사용되는 구성의 **가상서버를 쉽고 빠르게 구축하기 위한 서비스**
        - 응용 프로그램 배포나 세부 설정은 사용자가 수행
    - 여러 인스턴스 만들면 자동으로 로드밸런싱, SSL/TLS 인증서를 통한 통신 암호화 대응 가능
    - DB나 컨테이너 환경 생성에 대응 가능
    - VPC 를 통해 다른 AWS 서비스와도 연동 가능
    - 미들웨어 버전 고려하여 설치, ELB 를 이용한 유연한 부하 분산의 자유도는 없음
    → EC2 로 마이그레이션 해야 함
- **AWS Elastic Beanstalk**
    - java, .NET, PHP, Python, Docker 와 같은 주요 언어나 환경에서 개발된 **응용 프로그램을 배포하기 위한 실행 환경을 자동으로 만들어주는 서비스**
        - 응용 프로그램 배포까지 beanstalk 이 수행
    - 플랫폼 지정해서 응용 프로그램 코드 업로드하면, AWS 자원이 자동 구축되고 응용 프로그램이 배포됨
    - EC2, RDS 같은 AWS 서비스가 이용되므로 사용자가 스스로 구축할 수 있는 시스템을 서버 지식 없이도 구축 가능
    - 여러가지 배포 방식을 지원
        - All at once : 모든 인스턴스 응용 프로그램 `한번에 교체` / 교체 중에는 서비스 종료
        - 롤링 : 각 인스턴스 응용 프로그램을 `순서대로 교체` / 교체 중인 인스턴스만 서비스가 정지
        - immutable : 새로운 응용 프로그램을 배포한 인스턴스 만든 후 `기존 인스턴스 삭제`
- **AWS App Runner**
    - **컨테이너화된** 응용 프로그램을 **손쉽게 배포**하는 서비스
    - 컨테이너 이미지 지정, 서비스 포트나 auto scaling 과 같은 설정만으로 **컨테이너를 fargate 에 배포 가능**
- **AWS Batch**
    - 지정된 프로그램을 **EC2나 Fargate 에서 실행**하는 서비스
    - lambda 와 비슷해 보이지만, **실행 프로그램의 순서 관리 / 다수 프로그램의 병렬 구동과 같은 복잡한 제어에 특화된 서비스**
- **AWS Outposts**
    - 온프레미스 환경에서 AWS 와 같은 서비스를 실행할 수 있도록 **AWS가 물리 서버를 대여하는 서비스** (AWS 를 `프라이빗 클라우드화` 해서 제공하는 서비스)
    - 데이터 센터에 설치할 수 있는 서버 기기 세트가 제공되고, 해당 기기를 AWS 와 인터넷으로 연결하면 데이터 선터에 AWS 일부 서비스를 가져온 것처럼 이용 가능
        - AWS Outposts 를 `AWS 의 새로운 가용 영역으로 사용 가능`
