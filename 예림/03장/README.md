...
- 서버 지식 하나 없이 자신이 만든 응용 프로그램을 바로 실행해볼 수 있는 적절한 구성을 즛기 제공하는 간단한 컴퓨팅 서비스를 살펴보자

### AWS Lightsail

- 자주 사용하는 구성의 가상 서버를 쉽고 빠르게 구축하기 위한 서비스
- 장점 : 서비스 자체는 단순하지만 있을 건 다 있음. 여러 인스턴스를 만들면 자동으로 로드 밸런싱을 해주거나 SSL/TLS 인증서를 통한 통신 암호화도 대응 가능
- 단점 : 미들웨어 버전을 고려해 설치하거나 ELB를 이용해 유연한 부하 분산을 설정하는 등의 자유도는 없음.

### AWS Elastic Beanstalk

- `AWS Elastic Beanstalk`: Java, .NET, PHP, Python, Ruby, Go, Docker와 같은 주요 언어나 환경에서 개발한 응용 프로그램을 배포하기 위한 **실행 환경을 자동으로 만들어주는 서비스**
- 플랫폼을 선택하고 코드를 업로드하면 응용 프로그램을 실행할 수 있음.
    - EC2, RDS, S3와 같은 AWS 자원 자동 구축
- EC2나 RDS와 같은 AWS 서비스가 이용되므로 사용자가 스스로 구축할 수 있는 시스템을 서버 지식 없이도 구축 가능
- 여러가지 배포 방식 지원
    - ALL at once 방식 : 모든 인스턴스의 응용 프로그램을 한 번에 교체하는 방식. 교체 중에는 서비스 중지
    - 롤링 방식 : 각 인스턴스의 응용 프로그램을 순서대로 교체하는 방식. 교체중인 인스턴스만 서비스 정지
    - 이뮤터블(불변)방식 : 새로운 응용 프로그램을 배포한 인스턴스를 만든 뒤 기존 인스턴스를 삭제하는 방식. 서비스를 계속 중지하면서 문제가 발생하면 쉽게 배포 중지 가능

### AWS App Runner

- `AWS App Runner` : 컨테이너화된 응용 프로그램을 쉽게 배포하는 서비스
- 몇 가지 설정만으로 컨테이너를 Fargate에 배포 가능
- 나온지 얼마 안됨. 기대 잔뜩 받는 중

### AWS Batch

- `AWS Batch` : 지정된 프로그램을 EC2나 Fargate에서 실행하는 서비스
- 실행 프로그램의 순서 관리나 다수 프로그램의 병렬 구동과 같은 복잡한 제어에 특화된 서비스

### 4가지 비교

| Lightsail | Beanstalk | App Runner | AWS Batch |
| --- | --- | --- | --- |
| 지정된 미들웨어를 설치한 가상 서버 제공 | EC2나 RDS를 사용해 지정한 미들웨어를 동작시킬 실행환경 구축 | 지정된 컨테이너가 Fargate에서 실행 | 지정한 프로그램을 EC2 등에서 실행 |
| 응용 프로그램의 프로그램 배포나 세부 설정은 사용자가 수행 | 응용 프로그램 배포까지 Beanstalk가 수행 | 부하 분산 기능이나 Auto Scaling 기능도 자동으로 설정 | 설정에 따라 부팅 순서 제어, 병렬 구동 등이 자동으로 수행 |

### AWS Outposts

- `AWS Outposts` : 사용자가 온프레미스 환경에서 AWS와 같은 서비스를 실행할 수 있도록 AWS가 물리 서버를 대여하는 서비스
    - AWS를 **프라이빗 클라우드화**해서 제공
- 데이터 센터에 설치할 수 있는 서버기기 세트 제공
- 내부에는 EC2, RDS, S3 와 같은 서비스가 동작하게 만들어짐.
- 이를 데이터 센터에 설치해 AWS와 인터넷으로 연결하면 데이터 센터에 AWS 일부 서비스를 가져온 것처럼 이용 가능
- AWS Outpost를 AWS의 새로운 가용영역으로 사용 가능
